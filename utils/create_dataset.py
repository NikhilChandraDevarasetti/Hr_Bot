...
def fetch_response():
    ...
    for records in nlu_data:
        ...
        if records['response'][0]=='buttons':
            for i in records['response'][2:]:
                for j in i:
                    a = "utter_" + records["intent"]
                    if a not in responses:

                        responses[a] = [{'text':records['response'][1],"buttons": [{'title':j[0],'payload':j[1]}]}]
                    else:
                       responses[a][0]['buttons'].append({'title':j[0],'payload':j[1]})


        if type(records['response'][0]) == str:
            print(records['response'][0])
            for i in records['response'][0:]:
                a = "utter_" + records["intent"]
                if a not in responses:
                    responses[a] = [{"text": i}]
